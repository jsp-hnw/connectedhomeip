...
class IMXBuilder(GnBuilder):
    ...
    def GnBuildArgs(self):
        ...
        toolchain_root = self.SysRootPath('TOOLCHAIN_ROOT')
        toolchain_sysroot = self.SysRootPath('TOOLCHAIN_SYSROOT')
        humax_sysroot = self.SysRootPath('HUMAX_SYSROOT')
        target_cpu = 'arm'
        arm_arch = 'armv7-a'
        args = [
            ...
            f'target_cflags=[\
                ...
                "-Wl,-rpath-link,{humax_sysroot}/lib/",\
                "-Wl,-rpath-link,{humax_sysroot}/usr/lib/",\
                "-Wl,-rpath-link,{toolchain_sysroot}/lib/",\
                "-Wl,-rpath-link,{toolchain_sysroot}/usr/lib/",\
                ]',
            f'target_ldflags=[\
                "-L{humax_sysroot}/lib/",\
                "-L{humax_sysroot}/usr/lib/",\
                "-L{toolchain_sysroot}/lib/",\
                "-L{toolchain_sysroot}/usr/lib/",\
                "-Wl,-rpath-link,{humax_sysroot}/lib",\
                ]',
            f'target_cc="{toolchain_root}/bin/arm-buildroot-linux-gnueabi-gcc"',
            f'target_cxx="{toolchain_root}/bin/arm-buildroot-linux-gnueabi-g++"',
            f'target_ar="{toolchain_root}/bin/arm-buildroot-linux-gnueabi-ar"',
        ]

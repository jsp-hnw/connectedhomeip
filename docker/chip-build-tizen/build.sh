...
[[ ${*/--skip-build//} != "${*}" ]] || {
    docker build "${BUILD_ARGS[@]}" --build-arg TARGETPLATFORM="$TARGET_PLATFORM_TYPE" --build-arg USER_NAME="$USER" --build-arg USER_UID="$UID" --build-arg VERSION="$VERSION" -t "$IMAGE" .
